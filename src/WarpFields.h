...
namespace WarpFields {
   ...
   * ---
   *
   * A custom "Supports::CLASS" class must implement:
   *  float CLASS::at(float x, float y) const
   * which returns a scalar value in [0,1] indicating how strongly any given
   * pixel coordinate is influenced by a given instance of CLASS.
   */
  namespace Supports {
   ...
   * ---
   *
   * A custom "Displacers::CLASS" class must implement
   *   V2 raw_flow_at(float x, float y) const
   *   V2 raw_iflow_at(float x, float y) const
   * which produce forward resp. backward 2D flow vectors (V2 is 
   * "std::tuple<float,float>") for a given pixel. These are UNWEIGHTED, i.e.
   * NOT influenced by "Supports" elements.
   */
