...
namespace WarpFields {
   ...
   * displacer yields a valid, but boring flow field. Each displacer must be
  ...
  FlowField& FlowField::init_from_DisplacementComposer(const DisplacementComposer& dc)
  {
    ...
    for (int iter = 17; iter > 0; --iter) {
      ...
      cimg_forXY(from_ref,x,y) {
        ...
        if (x+fx < 0 or x+fx > W+2*skirt-1 or 
            y+fy < 0 or y+fy > H+2*skirt-1) {
    ...
    ///
    cimg_forXY(flow,x,y) {
      if (x+flow(x,y,0) < 0 or x+flow(x,y,0) >= W+2*skirt or 
          y+flow(x,y,1) < 0 or y+flow(x,y,1) >= H+2*skirt) {
        flagged(x,y) = (unsigned char)255;
      }
      if (flagged(x,y)) {
    ...
    for (int iter = 17; iter > 0; --iter) {
      ...
      cimg_forXY(from_ref,x,y) {
        const float fx = from_ref(x,y,0);
        const float fy = from_ref(x,y,1);
        if (x+fx < 0 or x+fx >= W+2*skirt or 
            y+fy < 0 or y+fy >= H+2*skirt) {
          flagged(x,y) = (unsigned char)255;
          to_ref(x,y,0) = fx;
          to_ref(x,y,1) = fy;
          continue;
    ...
    cimg_forXY(iflow,x,y) {
      if (x+iflow(x,y,0) < 0 or x+iflow(x,y,0) >= W+2*skirt or 
          y+iflow(x,y,1) < 0 or y+iflow(x,y,1) >= H+2*skirt) {
        flagged(x,y) = (unsigned char)255;
      }
      if (flagged(x,y)) {
    ...
    flow.crop(skirt,skirt,
                       W+skirt,H+skirt);
    iflow.crop(skirt,skirt,
                       W+skirt,H+skirt);
